    ...
    try {
        ...
        if (trainer) {
            const user = await User.findOne({
                fullname: { $regex: trainer, $options: "i" }
            });
            if (user) {
        ...
        const classes = await ClassSession.find(filter).populate("trainer", "fullname");
        if (available === "true") {
...
export const getClassById = async (req, res) => {
    try {
        const { id } = req.params;
        const classSession = await ClassSession.findById(id).populate("trainer", "fullname");
        if (!classSession) {
            return res.status(404).json({
                message: "Class not found "
            });
